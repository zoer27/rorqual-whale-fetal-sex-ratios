...
file<-"Code/Stan Code/MisID_Brydes.stan" #limited L50 and delta to improve convergence 
...
runStanMisID(brydes_bin)


#checking convergence
Br_mod1<-readRDS("Results/StanMCMC/Brydes_mod1.RDS")
Br_mod2<-readRDS("Results/StanMCMC/Brydes_mod2.RDS")
Br_mod3<-readRDS("Results/StanMCMC/Brydes_mod3.RDS")

parsofint<-c("a10", "b")
print(mcmc_trace(Br_mod1$draws(parsofint), facet_args = list(ncol = 2)))

parsofint2<-c("a10", "b", "L50", "delta")
print(mcmc_trace(Br_mod2$draws(parsofint2), facet_args = list(ncol = 2)))
print(mcmc_trace(Br_mod3$draws(parsofint2), facet_args = list(ncol = 2)))
